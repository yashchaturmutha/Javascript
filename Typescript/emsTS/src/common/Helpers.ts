import { RoleBUHead, RoleFinance, RoleHR } from "./constants";
...
export class Helpers {
  ...
  static IsRoleHR(employeeDetailRoles:number[]):boolean {
    if(employeeDetailRoles.indexOf(RoleHR) >=0 )
    {
        return true;
    }
    return false;
  ...
  static IsBuHead(employeeDetailRoles:number[]):boolean {
    if(employeeDetailRoles.indexOf(RoleBUHead) >=0 )
    {
        return true;
    }
    return false;
  }
  static IsFinance(employeeDetailRoles:number[]):boolean {
    if(employeeDetailRoles.indexOf(RoleFinance) >=0 )
    {
        return true;
    }
    return false;
